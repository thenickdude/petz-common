...
type
  TtkTokenKind = (tkComment, tkIdentifier, tkKey, tkNull, tkNumber,
    tkPreprocessor, tkSpace, tkString, tkSymbol, tkUnknown,tkSectionHeading,tkVariation);
...
type
  TSynLNZSyn = class(TSynCustomHighlighter)
  private
    fRange: TRangeState;
    fLine: PChar;
    ...
    fLineNumber : Integer;
    fVariationAttri:TSynHighlighterAttributes;
  ...
  published
    property SectionAttri: TSynHighlighterAttributes read fsectionattri
      write fsectionattri;
    property VariationAttri: TSynHighlighterAttributes read fvariationattri
      write fvariationattri;
...
uses
  ...
  procedure Register;
  begin
    RegisterComponents('Petz', [TSynLNZSyn]);
  end;
...
function TSynlnzsyn.IsKeyword(const AKeyword: string): boolean;             //mh 2000-11-08
var
...
procedure TSynlnzsyn.MakeMethodTables;
var
  I: Char;
begin
  for I := #0 to #255 do
    case I of
      '#': fProcTable[I] := variationproc;
...
constructor TSynlnzsyn.Create(AOwner: TComponent);
begin
  ...
  fvariationattri:= TSynHighlighterAttributes.Create(SYNS_AttrSpecialVariable);
  fvariationattri.Foreground:=clred;
  AddAttribute(fvariationattri);
...
destructor TSynlnzsyn.Destroy;
begin
  fKeyWords.Free;
  inherited Destroy;
end; { Destroy }

procedure TSynlnzsyn.SetLine(NewValue: String; LineNumber:Integer);
begin
...
procedure TSynlnzsyn.PasStyleProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynlnzsyn.CStyleProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynlnzsyn.variationproc;
begin
    fTokenID := tkvariation;
    frange:=rspasstyle;
    repeat
      inc(Run);
    until fLine[Run] in [#0, #10, #13];
...
procedure TSynlnzsyn.SectionOpenProc;
begin
    fTokenID := tkSectionHeading;
    fRange := rsPasStyle;
    inc(Run);
...
procedure TSynlnzsyn.LFProc;
begin
  fTokenID := tkSpace;
  inc(Run);
end;

procedure TSynlnzsyn.NullProc;
begin
  fTokenID := tkNull;
end;

procedure TSynlnzsyn.NumberProc;
begin
  inc(Run);
  fTokenID := tkNumber;
...
procedure TSynlnzsyn.RoundOpenProc;
begin
  inc(Run);
   fTokenId := tkSymbol;
end;

procedure TSynlnzsyn.semiProc;
begin
        inc(Run, 1);
        fTokenID := tkComment;
        while FLine[Run] <> #0 do
...
procedure TSynlnzsyn.SpaceProc;
begin
  inc(Run);
  fTokenID := tkSpace;
...
procedure TSynlnzsyn.StringProc;
begin
  fTokenID := tkString;
...
procedure TSynlnzsyn.UnknownProc;
begin
  inc(Run);
  fTokenID := tkUnKnown;
end;

procedure TSynlnzsyn.Next;
begin
  fTokenPos := Run;
  case fRange of
    rsAnsi: AnsiProc;
...
function TSynlnzsyn.GetDefaultAttribute(Index: integer): TSynHighlighterAttributes;
begin
  case Index of
...
function TSynlnzsyn.GetEol: Boolean;
begin
  Result := fTokenId = tkNull;
end;

function TSynlnzsyn.GetRange: Pointer;
begin
  Result := Pointer(fRange);
end;

function TSynlnzsyn.GetToken: String;
var
  Len: LongInt;
begin
  Len := Run - fTokenPos;
...
function TSynlnzsyn.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
end;

function TSynlnzsyn.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case fTokenID of
    tkVariation:result:=fvariationattri;
    tkSectionHeading: result:=fSectionAttri;
...
function TSynlnzsyn.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;

function TSynlnzsyn.GetTokenPos: Integer;
begin
  Result := fTokenPos;
end;

procedure TSynlnzsyn.ReSetRange;
begin
  fRange := rsUnknown;
end;

procedure TSynlnzsyn.SetRange(Value: Pointer);
begin
  fRange := TRangeState(Value);
end;

procedure TSynlnzsyn.SetKeyWords(const Value: TStrings);
var
  i: Integer;
begin
  if Value <> nil then
...
{$IFNDEF SYN_CPPB_1} class {$ENDIF}
function TSynlnzsyn.GetLanguageName: string;
begin
  Result := SYNS_LangGeneral;
end;

{$IFNDEF SYN_CLX}
function TSynlnzsyn.LoadFromRegistry(RootKey: HKEY; Key: string): boolean;
var
  r: TBetterRegistry;
begin
...
function TSynlnzsyn.SaveToRegistry(RootKey: HKEY; Key: string): boolean;
var
  r: TBetterRegistry;
begin
  r:= TBetterRegistry.Create;
...
function TSynlnzsyn.GetStringDelim: TStringDelim;
begin
...
procedure TSynlnzsyn.SetStringDelim(const Value: TStringDelim);
var
  newCh: char;
begin
  case Value of
...
function TSynlnzsyn.GetIdentifierChars: string;
var
  ch: char;
  s: shortstring;
begin
...
procedure TSynlnzsyn.SetIdentifierChars(const Value: string);
var
  i: integer;
begin
  fIdentChars := [];
...
function TSynlnzsyn.GetIdentChars: TSynIdentChars;
begin
  Result := fIdentChars;
end;

procedure TSynlnzsyn.SetDetectPreprocessor(Value: boolean);
begin
...
initialization
  MakeIdentTable;
{$IFNDEF SYN_CPPB_1}                                                    
  RegisterPlaceableHighlighter(TSynLNZSyn);
{$ENDIF}
end.
